...
module.exports = {
  commands: ["reaction-roles", "reactionroles", "rrmake", "buttonroles"],
  permissions: ["MANAGE_GUILD", "ADMINISTRATOR"],
  permissionError: "You do not have permissions to do this!",
  run: async (message, args, text, client) => {
    const brmanager = new Nuggies.buttonroles();
    message.channel.send(
      "Send messages in `roleID color label emoji` syntax! Once finished say `done`. If you would like to cancel, simply just say `done`, and delete the bot's message."
    );
