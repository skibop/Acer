...
module.exports = {
  commands: "dropdownroles",
  run: async (message, args, text, client) => {
    if (!message.member.hasPermission("MANAGE_SERVER"))
      return message.reply("You do not have the permission `MANAGE_SERVER`");
    const dpmanager = new Nuggies.dropdownroles();
    message.channel.send(
      "Send messages in `roleID label emoji` syntax! Once finished say `done`. If you would like to cancel, simply just say `done`, and delete the bot's message."
    );
