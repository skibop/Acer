  ...
  run: async (message, args, text, client) => {
    ...
    if (!doggo.permissions.has(perms))
      return message.reply(
        ` I do not have permission to ban users pls enable permission **\`MANAGE_NICKNAMES\`** for me`
      );

    if (!user)
      return message.reply(
        ` Please mention or provide the ID of the user from this guild !! **\`-nickname [Mention or ID] [The Nickname]\`**`
      );

    if (!args[1]) return message.reply(`Please provide a nickname`);
    ...
    if (nickname.startsWith('"')) {
      ...
      if (!nickname.includes('"'))
        return message.reply(
          `Please ensure the nickname is surrounded in quotes ""`
        );

      if (user.roles.highest.position > message.member.roles.highest.position)
        return message.reply(
          ` You cannot ban someone with an equal or higher role to you !!! or if you are owner pls be yourself in a higher position`
        );

      if (user.roles.highest.position > doggo.roles.highest.position)
        return message.reply(
          ` You cannot ban someone with an equal or higher role than me !!`
        );
    ...
    if (nickname.length > 32) {
      return message.reply(
        `Provided nickname is too big pls provide a nickname which is lesser than 32 characters !!`
      );
    } else {
      try {
        ...
        const embed = new MessageEmbed()
          ...
          .setDescription(
            ` <@${user.id}> (\`${user.user.tag}\`) nickname has been successfully changed`
          )
          .addField(
            "Changed By",
