...
module.exports = {
  ...
  run: async (message, args, text, client) => {
    ...
    if (user.bot) return message.channel.send("This user is a bot.");
    if (!client.users.cache.get(user.id) || !user)
    ...
    if (amount.includes("-"))
      return message.channel.send("You can't send negitive money.");
