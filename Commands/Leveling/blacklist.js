  ...
  run: async (message, args, text, client) => {
    ...
    if (!args.length)
      return message.reply(
        `Require arguments: \`User\` or \`Channel\` or \`List\``
      );

    if (args[0].toLowerCase() == "list") {
      const currentPage = parseInt(args[1]) || 1;
      const Lists = sql
        .prepare("SELECT * FROM blacklistTable WHERE guild = ?")
        .all(message.guild.id);
      if (parseFloat(args[0]) > Math.ceil(Lists.length / 10)) {
        return message.reply(
          `Invalid page number! There are only ${Math.ceil(
            Lists.length / 10
          )} pages`
        );
      }
      ...
      let user =
        ...
        message.guild.members.cache.find(
          (x) =>
            x.user.username.toLowerCase() === args.slice(1).join(" ") ||
            x.user.username === args[1]
        );
      if (!args[1]) return message.reply(`Please mention an user!`);
      ...
      if (args[2] && args[2].toLowerCase() == "remove") {
        if (!ifExists.get(`${message.guild.id}-${user.id}`))
          ...
          sql
            .prepare("DELETE FROM blacklistTable WHERE id = ?")
      ...
      if (ifExists.get(`${message.guild.id}-${user.id}`))
        ...
        sql
          .prepare(
            "INSERT OR REPLACE INTO blacklistTable (guild, typeId, type, id) VALUES (?, ?, ?, ?);"
          )
          .run(
            message.guild.id,
            user.id,
      ...
      if (!args[1]) return message.reply(`Please mention a channel!`);
      if (!channel) return message.reply(`Cannot find channel!`);
      if (args[2] && args[2].toLowerCase() == "remove") {
