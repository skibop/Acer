...
module.exports = {
  ...
  run: async (message, args, text, client) => {
    ...
    if (!args[1])
      return message.reply("Please provide the time of the giveaway!", {
        allowedMentions: { repliedUser: false },
      });
    if (!ms(args[0]))
      return message.reply("Please provide a valid time! Example: `1m 1w`", {
        allowedMentions: { repliedUser: false },
      });
    if (!args.slice(2).join(" "))
      return message.reply("Please provide the prize!", {
        allowedMentions: { repliedUser: false },
      });
