...
module.exports = {
  ...
  run: async (message, args, text, client) => {
    if (!message.member.hasPermission("MANAGE_GUILD"))
      return message.reply("You are not allowed to create giveaways!");
    ...
    collector.on("collect", async (msg) => {
      ...
      if (step == 1) {
        ...
        message.channel.send(
          `The host is ${host}! Now do you want any requirements for the giveaway? Respond with yes or no.`
        );
      } else if (step == 6) {
      ...
      if (reason == "done" && giveaway.requirements.enabled) {
        ...
        rcollector.on("end", async (msg, r) => {
          ...
          if (r == "done") {
            ...
            await message.channel.send("Created a giveaway!");
          }
        });
      } else {
        Nuggies.giveaways.create({
