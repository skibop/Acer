...
client.on("ready", async () => {
  ...
  // Blacklist table
  const blacklistTable = sql
    .prepare(
      "SELECT count(*) FROM sqlite_master WHERE type='table' AND name = 'blacklistTable';"
    )
    .get();
  if (!blacklistTable["count(*)"]) {
    sql
      .prepare(
        "CREATE TABLE blacklistTable (guild TEXT, typeId TEXT, type TEXT, id TEXT PRIMARY KEY);"
      )
      .run();
  }
  // Settings table
  const settingsTable = sql
    .prepare(
  ...
  if (!settingsTable["count(*)"]) {
    sql
      .prepare(
        "CREATE TABLE settings (guild TEXT PRIMARY KEY, levelUpMessage TEXT, customXP INTEGER, customCooldown INTEGER);"
      )
      .run();
  }

  const channelTable = sql
    .prepare(
      "SELECT count(*) FROM sqlite_master WHERE type='table' AND name = 'channel';"
    )
    .get();
  if (!channelTable["count(*)"]) {
    sql
      .prepare("CREATE TABLE channel (guild TEXT PRIMARY KEY, channel TEXT);")
      .run();
  }
});
...
client.on("message", (message) => {
  ...
  if (
    blacklist.get(`${message.guild.id}-${message.author.id}`) ||
    blacklist.get(`${message.guild.id}-${message.channel.id}`)
  )
    return;
  ...
  let customSettings = sql
    .prepare("SELECT * FROM settings WHERE guild = ?")
    .get(message.guild.id);
  let channelLevel = sql
    .prepare("SELECT * FROM channel WHERE guild = ?")
    .get(message.guild.id);
  ...
  if (talkedRecently.get(message.author.id)) {
    ...
    if (level.xp >= nextXP) {
      ...
      if (!customSettings) {
        ...
        embed.setDescription(
          antonymsLevelUp(customSettings.levelUpMessage.toString())
        );
      ...
      try {
        if (!channelLevel || channelLevel.channel == "Default") {
          ...
          if (
            !permissionFlags.has("SEND_MESSAGES") ||
            !permissionFlags.has("VIEW_CHANNEL")
          )
            return;
          channel.send(embed);
        }
    ...
    setTimeout(() =>
      talkedRecently.delete(message.author.id, Date.now() + getCooldownfromDB)
    );
  }
